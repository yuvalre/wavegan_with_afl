...
import loader
from wavegan_afl import WaveGANGenerator, WaveGANGeneratorAFL, WaveGANDiscriminator

# TODO: add AFL variables to top scope and save that, feed it to 
"""
  Trains a WaveGAN
"""
def train(fps, args):
...
"""
  Trains the adversarial feedback loops of the WaveGAN
"""
def train_afl(fps, args):
    with tf.name_scope('loader'):
        x = loader.decode_extract_and_batch(
            fps,
            batch_size=args.train_batch_size,
    ...
    # Make generator
    with tf.variable_scope('G'):
        with tf.variable_scope('AFL'):
            alpha0 = tf.Variable(args.afl_alpha0)
            alpha1 = tf.Variable(args.afl_alpha1)
            alpha2 = tf.Variable(args.afl_alpha2)
            alpha3 = tf.Variable(args.afl_alpha3)
            alpha4 = tf.Variable(args.afl_alpha4)
        G_z = WaveGANGenerator(z, train=True, **args.wavegan_g_kwargs)
        if args.wavegan_genr_pp:
            with tf.variable_scope('pp_filt'):
                G_z = tf.layers.conv1d(G_z, 1, args.wavegan_genr_pp_len, use_bias=False, padding='same')
    G_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='AFL')
    ...
    # Make real discriminator
    with tf.name_scope('D_x'), tf.variable_scope('D'):
        D_x = WaveGANDiscriminator(x, train=True, **args.wavegan_d_kwargs)  # TODO should train be True?
    D_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='D')
    ...
    # Make fake discriminator
    with tf.name_scope('D_G_z'), tf.variable_scope('D', reuse=True):
        D_G_z = WaveGANDiscriminator(G_z, train=True, **args.wavegan_d_kwargs)

    # Make generator + AFL
    for step in range(1, args.train_loop_count):
        with tf.variable_scope('G', reuse=True):
            G_z = WaveGANGeneratorAFL(z, train=True, **args.wavegan_g_kwargs)
            if args.wavegan_genr_pp:
                with tf.variable_scope('pp_filt'):
                    G_z = tf.layers.conv1d(G_z, 1, args.wavegan_genr_pp_len, use_bias=False, padding='same')
        with tf.name_scope('D_G_z'), tf.variable_scope('D', reuse=True):
...
def infer_afl(args):
    ...
    with tf.name_scope('D_G_z'), tf.variable_scope('D'):
        D_G_z = WaveGANDiscriminator(G_z, train=False, **args.wavegan_d_kwargs)

    # Execute generator + AFL
    for step in range(1, args.infer_loop_count):
        with tf.variable_scope('G', reuse=True):
            G_z = WaveGANGeneratorAFL(z, train=False, **args.wavegan_g_kwargs)
            if args.wavegan_genr_pp:
                with tf.variable_scope('pp_filt'):
                    G_z = tf.layers.conv1d(G_z, 1, args.wavegan_genr_pp_len, use_bias=False, padding='same')
        with tf.name_scope('D_G_z'), tf.variable_scope('D', reuse=True):
...
if __name__ == '__main__':
    ...
    afl_args = parser.add_argument_group('AFL')
    afl_args.add_argument('--afl_alpha0', type=float,
                            help='Alpha 0')
    afl_args.add_argument('--afl_alpha1', type=float,
                          help='Alpha 1')
    afl_args.add_argument('--afl_alpha2', type=float,
                          help='Alpha 2')
    afl_args.add_argument('--afl_alpha3', type=float,
                          help='Alpha 3')
    afl_args.add_argument('--afl_alpha4', type=float,
                          help='Alpha 4')
    ...
    train_args.add_argument('--train_loop_count', type=int,
                            help='How many feedback loops to perform')

    infer_args = parser.add_argument_group('Inference')
    infer_args.add_argument('--infer_loop_count', type=int,
                              help='How many feedback loops to perform')
